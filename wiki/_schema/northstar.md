...
- `README` — the human landing page; the place to start for orientation and usage
- `NORTHSTAR` — the project charter and routing contract; source of truth for the Published tree shape
- `Intake` — raw capture lane for incoming content not yet normalized or fully understood
- `Staged` — high-confidence content prepared for human review or publish
- `Review` — human arbitration lane for ambiguity, conflict, or risk
- `Published` — canonical output surface organized by the tree blueprint above
- `Admin` — machine-managed structure: `_templates`, `_registry`, `_audit`
