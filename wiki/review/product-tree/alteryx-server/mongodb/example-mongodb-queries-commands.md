...
| --- | --- |
| **remove()** | remove() is deprecated in Mongo 6.0  Use the following instead to delete all records: |
...
| Search with a “contains” function to match a portion of a string, rather than an exact match of the full string | Forward-Slashes at the beginning and end of the string to search for, in place of the quotation marks. Spaces in the string do not need quotes or escape characters. |  |
| Search with case-insensitivity if unsure whether characters are upper or lowercase. | Forward-Slashes at the beginning and end of the string to search for, in place of the quotation marks, with an “i” at the end. |  |
| Search for values that are **not** null | List the field name in quotation marks with a colon, then the non-null value. The example will return all public workflows, since the value is otherwise null if not set. |  |
