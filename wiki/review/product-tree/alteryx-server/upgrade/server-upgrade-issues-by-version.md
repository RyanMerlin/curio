...
updated_at: 2026-04-14T15:20:40Z
confidence: 0.9
cross_refs: []
...
|  |  |
|  | # |
| # 24.2  ### Mongo 7.0 upgrade  ### New AMP Recommendations (which can be applied to old versions) |  |
|  |  |
...
|  |  |
|  | # |
| # 24.1  ### Timestamps moved to UTC  ### Python upgrade to 3.10  ### appInfos.PublishedRevision merged into Revisions array |  |
|  |  |
...
| **Prevent** | The issue can’t be prevented, but it can be addressed immediately after upgrade (**BEFORE **workflows are manually run as that loses the original run count).     - Upgrade to experience the issue    - Run the workflow attached to the Jira ticket    - Run the Mongo update queries generated by the workflow |
...
| **Prevent** | For Customers who have:     - Server default is Safe or Semi-Safe    - Admins manually set workflow settings to Semi-Safe or Unrestricted  Mongo query can be developed to set the **appInfos.PublishedRevsion.ExecutionMode** to the value of **Revisions[0].ExecutionMode **BEFORE upgrade to avoid the workflows reverting to Safe mode.  Note: not sure how to handle the “published” version not being the latest version. |
...
| **Versions** | Affects     - 24.1 (experienced with 24.1_P6 but may occur in earlier patches) |
...
|  |  |
|  | #### |
| # 23.2  ### Mongo 6.0 upgrade  ### __ServiceData blob removed     - Why was the __ServiceData field removed and how can I retrieve data contained in the __ServiceData blob? (KB)    - A few BLOBs remain using a different field an,e  theycan still be unpacked with ServiceDataParser.yxmc,Mongo Input Tool       - Mongo Input Tool |  |
|  |  |
| **23.2** | ## activeRed preventGreen Mongo upgrade 6.0 - Unexpected '4.0.10' in ASMongoDBVersion.bin |
| **Prevent** | Both checks are required as we see that **ASMongoDBVersion.bin** will hold on to an old setting (**4.0.10**) regardless of the Alteryx Server version     1. Confirm we ARE upgrading from a 4.2.X version of Mongo based on the Server versionASMongoDBVersion.bin    ASMongoDBVersion.bin    2. Confirm the content of ASMongoDBVersion.bin is correctly set to either    4.2.15    4.2.22 |
| **Error** | Occurs in last step of the upgrade installation, before attempting to start the Service  **Alteryx Server Database Migration** **Could not start previous version of MongoDB: The MongoDB database failed to start with exit code: 14.** |
...
| **KB** | [Alteryx Server Upgrade "Error: Could not start previous version of MongoDB: The MongoDB database failed to start with exit code: 14."](https://knowledge.alteryx.com/index/s/article/Alteryx-Server-upgrade-Error-Could-not-start-previous-version-of-MongoDB-The-MongoDB-database-failed-to-start-with-exit-code-14) (KB) |
|  |  |
| **23.2** | ## activeRed Mongo upgrade 6.0 - Missing ASMongoDBVersion.bin |
| **Error** | Occurs in last step of the upgrade installation, before attempting to start the Service  **Alteryx Server Database Migration** **Could not finalize MongoDB restore. The MongoDB database failed to start with exit code: 100.** |
...
| **Resolution** | Recreate missing **ASMongoDBVersion.bin**     - Could not finalize mongodb restore the mongodb database failed to start with exit code 100 (KB) |
...
|  |  |
|  | #### |
| # 23.1  ### JS Framework Replaced  ### Lucene Replaced |  |
|  |  |
...
| **Issue** | Indexing errors due to the indexing system being replaced cause failures after Service starts or the appearance of missing users or other data     - Reindex MongoDB |
|  |  |
| **23.1** | ## activeRed Embedded R upgrade |
| **Issue** | Embedded R version upgrades from 4.1.3 to 4.2.3 in RInstaller_2023.1.1.200.exe+ requiring customers to update R code     - R Tool + |
|  |  |
...
| **Error** | Please Sign In  The page you are trying to reach requires you to sign in before you can view it. |
| **KB** | [Exception thrown when asserting SAML response from IDP](https://knowledge.alteryx.com/index/s/article/Exception-thrown-when-asserting-SAML-response-from-IDP-when-using-Meta-or-x509) (KB) |
...
| **Error** | Page Not Found  The page you are trying to reach does not exist. |
| **Log** | AAS/SSO log  ``` Saml2 Status Code: Requester->  Saml2 Status Message: UnknownAssertionConsumerServiceURL https://{FQDN}/webapi/saml2/acs->     Saml2 Second Level Status: ->   at Sustainsys.Saml2.Saml2P.Saml2Response. ]]> ``` |
| **KB** | [Alteryx SAML auth error after upgrade to 2023 isUnauthorized=true](https://knowledge.alteryx.com/index/s/article/Alteryx-SAML-auth-error-after-upgrade-to-2023-isUnauthorizedtrue) (KB) |
| **Veraions** | 22.3,  23.2 |
| **Jira** | TCPE-94077dcf2c9-72f3-3ff6-8e99-fe88e9f473f1System Jira [**22.3.1_Patch10, 23.1.1_Patch7, 23.2.1_Patch4, 24.1**] |
|  |  |
|  | #### |
| # 22.1  ### Encryption Key Transfer |  |
|  |  |
...
|  |  |
| **22.1** **Patch_3** | ## ActiveRed PreventGreen (22.1_Patch3 - 24.1) Migration to version 40.03 failed: Renaming collection from 'dataConnections' to 'dataConnectionsMigrationInProcess_40.03 |
