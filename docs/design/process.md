...
The deterministic Confluence tree contract for `curio init` lives in [curio-core-init.md](curio-core-init.md).
...
For example, if the agent reads multiple sources about Example Server installation issues involving ODBC driver behavior across versions `23.2`, `24.2`, and `25.1`, the target proposal should lean toward a hierarchy such as:
