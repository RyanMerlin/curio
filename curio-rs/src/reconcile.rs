...
pub struct ReconcileDecision {
    /// Relative path segments for the target category, e.g. `["product-tree", "example-server"]`.
    ...
    /// Optional rewritten page body (markdown). When present, the apply
    /// path REPLACES the original intake body with this content. Use for
    /// agent-authored synthesis: the design doc requires the proposed
    /// page to be a curated knowledge object, not a raw capture.
    /// Backwards-compatible: omitted from older route files → behavior
    /// matches pre-rewrite-era (intake body preserved).
    #[serde(default)]
    pub proposed_body_markdown: Option<String>,
    /// Optional structured "decision section" prepended above the body
    /// after rewrite. Carries route, scores, alternatives, recommended
    /// action — what `process.md` §"Required proposal outputs" calls the
    /// "structured information section at the top of the page."
    #[serde(default)]
    pub decision_section_markdown: Option<String>,
    /// Self-reported classification of the rewrite the agent did.
    /// One of `none`, `light_edit`, `full_synthesis`. Stored in the
    /// proposal dossier for audit / metrics.
    #[serde(default)]
    pub body_rewrite_kind: Option<String>,
    /// When set, this intake page is being merged into the named primary
    /// slug from the same intake request (T2-C). The apply path will
    /// append this page's body to the primary's body (under a heading)
    /// and remove this page from intake. Used by the agent to consolidate
    /// multiple related sources into one curated proposal.
    #[serde(default)]
    pub merge_into_slug: Option<String>,
}
...
/// Returns a suggested subtree slug based on the operator-supplied product
/// registry. This is a HINT fed into the LLM prompt — not a routing
/// decision. The registry is the SSOT: it lives in
/// `wiki/_admin/config.yaml::products` and is the only thing that should
/// know which slugs / signals are meaningful for THIS deployment.
///
/// When the registry is empty (Curio's default state out-of-the-box) this
/// returns `None` — the binary makes no domain assumptions of its own.
/// Operators populate the registry to teach Curio about their products.
///
/// Matching order:
///   1. Title signals — iterated in registry order, first match wins.
///   2. Body signals — iterated in registry order, first match wins.
///      A product is skipped if any of its `body_signals_exclude_when`
///      strings is also present in the body.
pub fn heuristic_pre_signal(
    title: &str,
    body: &str,
    ...
    // Title wins over body — the dominant content topic outranks
    // incidental mentions buried in prose.
    for product in products {
        if product.slug.is_empty() {
            continue;
        }
        for sig in &product.title_signals {
            let lower = sig.to_lowercase();
            if !lower.is_empty() && title_text.contains(&lower) {
    ...
    for product in products {
        ...
        // Disqualify the product when any exclude-when string is present.
        let disqualified = product.body_signals_exclude_when.iter().any(|ex| {
            let lower = ex.to_lowercase();
            !lower.is_empty() && body_text.contains(&lower)
        });
        if disqualified {
            continue;
        }
        for sig in &product.body_signals {
            let lower = sig.to_lowercase();
            if !lower.is_empty() && body_text.contains(&lower) {
                return Some(product.slug.clone());
            }
        }
    }
...
pub async fn route_with_llm(
    ...
    let pre_signal = heuristic_pre_signal(title, body, products);
...
## Routing Rules
- Choose the MOST SPECIFIC matching route. Prefer a subtree (e.g. product-tree/example-server) over a top-level tree.
