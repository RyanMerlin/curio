...
pub struct Frontmatter {
    ...
    /// Per-invocation intake batch identifier. All pages produced by a
    /// single `curio intake` call share this id so downstream agents
    /// can group sources by request when deciding whether to merge or
    /// split. Backwards-compatible — older pages without it remain
    /// individual single-source proposals.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub intake_request_id: Option<String>,
}
...
mod source_kind_tests {
    ...
    fn origin_url_confluence_with_webui_path() {
        ...
        assert_eq!(
            kind.origin_url("https://example.invalid"),
            Some("https://example.invalid/wiki/spaces/TEST/pages/123/Title".into())
        );
    }
    ...
    fn origin_url_confluence_without_webui_path() {
        ...
        assert_eq!(
            kind.origin_url("https://example.invalid"),
            Some("https://example.invalid/wiki/pages/viewpage.action?pageId=456".into())
        );
    }
}
