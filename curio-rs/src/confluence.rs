...
impl ConfluenceClient {
    ...
    pub async fn create_or_update_page(
        ...
        let existing_page = if let Some(page) = existing_page {
            ...
            let outside_managed_root = match self.write_root_folder_id.as_deref() {
                Some(root_id) => !self.page_is_descendant_of(page_id, root_id).await?,
                None => false,
            };
    ...
    pub async fn get_space_details_v2(&self, space_key: &str) -> Result<Option<serde_json::Value>> {
        let url = format!("{}/api/v2/spaces?keys={}", self.base_url, space_key);
        let response = self
            .client
            .get(&url)
            ...
            .send()
            .await
            .context("Failed to fetch Confluence space details")?;
        let status = response.status();
        let text = response
            .text()
            .await
            .context("Failed to read Confluence space details response")?;
        if !status.is_success() {
            anyhow::bail!(
                "Failed to fetch Confluence space {}: {} — {}",
                space_key,
                status,
                text
            );
        ...
        let json: serde_json::Value =
            serde_json::from_str(&text).context("Failed to parse Confluence space details")?;
        Ok(json["results"]
            .as_array()
            .and_then(|results| results.first().cloned()))
    }

    /// Ensure a Confluence space homepage points at the provided page ID.
    /// Returns Ok(true) when an update was made, Ok(false) when no change was needed.
    pub async fn ensure_space_homepage(&self, space_key: &str, homepage_id: &str) -> Result<bool> {
        let Some(space) = self.get_space_details_v2(space_key).await? else {
            anyhow::bail!("Space {} not found", space_key);
        };
        if space["homepageId"].as_str() == Some(homepage_id) {
            return Ok(false);
        }
        let space_id = space["id"]
            .as_str()
            .context("Space ID missing from Confluence response")?;
        let name = space["name"].as_str().unwrap_or(space_key);
        let mut payload = serde_json::json!({
            "key": space_key,
            "name": name,
            "homepageId": homepage_id,
        });
        if let Some(description) = space["description"].as_str() {
        ...
        let response = self
            ...
            .send()
            .await
            .context("Failed to update Confluence space homepage")?;
        let status = response.status();
        let text = response
            .text()
            .await
            .context("Failed to read Confluence space update response")?;
        if status.is_success() {
            Ok(true)
        } else {
            anyhow::bail!(
                "Failed to update Confluence space homepage for {}: {} — {}",
                space_key,
                status,
                text
            );
