...
const CURIO_ROOT_STRUCTURE_PAGES: &[&str] = &["Intake", "Staged", "Review", "Published", "Admin"];

const CURIO_ADMIN_STRUCTURE_PAGES: &[&str] = &[
    "Northstar",
    "CURIO Readme",
    "Config",
    "Getting Started",
...
async fn validate_structure_pages(
    ...
    let admin_page = client
        .get_page_by_title(space_key, Some(root_page_id), "Admin")
        .await?;
    ...
    let expected_tree_pages = load_taxonomy(&config.wiki.wiki_dir)
        .map(|taxonomy| {
            taxonomy
                .nodes
                .into_iter()
...
fn resolve_env_value(
    ...
    match key {
        ...
        "CURIO_AUDIT_DIR" => ("${REPO_ROOT}/wiki/_admin".to_string(), ValueSource::Default),
