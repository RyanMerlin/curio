...
fn apply_approve(
    ...
    let filename = md_path
        .file_name()
        .with_context(|| format!("page path has no file name: {}", md_path.display()))?;
    ...
    if analysis.exists() {
        let analysis_name = analysis
            .file_name()
            .with_context(|| format!("analysis path has no file name: {}", analysis.display()))?;
        let dest_analysis = dest_dir.join(analysis_name);
