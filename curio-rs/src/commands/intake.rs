...
pub async fn run_intake(
    ...
    // Resume guard: a prior intake may have written files to wiki/intake/
    // but failed before the auto-commit (e.g. transient git error). Those
    // files are sitting uncommitted. Finalize them BEFORE this run so we
    // start from a clean tree and don't mistake them for "duplicates"
    // sitting in the registry without a corresponding git history.
    if !dry_run && config.wiki.auto_commit {
        finalize_pending_intake(wiki_dir)?;
    }
    ...
    // T2-C: every page from this single intake invocation gets the SAME
    // request_id stamped into its frontmatter, so the routing manifest
    // can group sources by request and the agent can decide
    // 1 → N proposals (merge multiple sources into one page, or split
    // a single source into several). Backwards-compatible — pages
    // without this field remain individual proposals.
    let request_id = format!("intake-{}", uuid::Uuid::new_v4());
    ...
    for item in items {
        ...
        let fm = Frontmatter {
            ...
            intake_request_id: Some(request_id.clone()),
        };
    ...
    if !dry_run && !ingested.is_empty() {
        ...
        let batch_id = generate_id(&format!("intake:{}:{}", batch_stamp, ingested.join("|")));
        let preview = ingested
            .iter()
            .take(5)
            .cloned()
            .collect::<Vec<_>>()
...
async fn collect_from_confluence(
    ...
    // Confluence URLs come in two flavors:
    //   .../wiki/spaces/<KEY>/pages/<id>/...        → page (with optional descendants)
    //   .../wiki/spaces/<KEY>/folder/<id>/...       → folder (always recursive)
    // Folders are not pages themselves; their descendants are what we ingest.
    let mut page_ids: Vec<String> = Vec::new();
    let mut children_map: std::collections::HashMap<String, Vec<String>> =
        std::collections::HashMap::new();
    let root_page_id: Option<String>;

    if let Some(folder_id) = extract_confluence_folder_id(url) {
        root_page_id = None;
        eprintln!("Detected Confluence folder URL — fetching descendants of folder {folder_id}...");
        let descendants = client.get_folder_descendants_v2(&folder_id).await?;
        for d in &descendants {
            // Only ingest page-typed descendants; skip nested folders (their pages are
            // already returned recursively by the descendants endpoint).
        ...
        if page_ids.is_empty() {
            anyhow::bail!(
                "Folder {folder_id} returned no page descendants — nothing to ingest. \
                 Verify the folder URL and that the bot user has read access."
            );
        }
        eprintln!("Fetching {} page(s) from folder...", page_ids.len());
    } else {
        let page_id = extract_confluence_page_id(url).ok_or_else(|| {
            anyhow::anyhow!(
                "Could not extract page or folder ID from Confluence URL: {}",
                url
            )
        })?;
        root_page_id = Some(page_id.clone());
    ...
    for (i, pid) in page_ids.iter().enumerate() {
        ...
        let is_root = root_page_id.as_deref() == Some(pid.as_str());
...
/// Commit any uncommitted files under `wiki/intake/` left over from a
/// prior intake run that crashed after writing files but before committing
/// (e.g. a transient `git add` failure). This makes intake idempotent across
/// crashes — the next run resumes by finalizing the pending batch instead
/// of leaving 16 orphan files dangling forever.
fn finalize_pending_intake(wiki_dir: &Path) -> Result<()> {
    let repo_root = wiki_dir.parent().unwrap_or(wiki_dir);
    let intake_rel_prefix = match wiki_dir
        .strip_prefix(repo_root)
        .ok()
        .and_then(|p| p.to_str())
    {
        Some(p) if !p.is_empty() => format!("{p}/intake/"),
        _ => "wiki/intake/".to_string(),
    };

    let porcelain = match crate::git_ops::git_status_porcelain(repo_root) {
        Ok(s) => s,
        Err(_) => return Ok(()),
    };
    // Lines look like " M path", "?? path", "A  path", etc. Trim status, check prefix.
    let has_pending = porcelain.lines().any(|line| {
        let path = line.get(3..).unwrap_or("").trim_start_matches('"');
        path.starts_with(&intake_rel_prefix)
    });
    if !has_pending {
        return Ok(());
    }
    ...
    if crate::git_ops::git_has_staged(repo_root) {
        crate::git_ops::git_commit(
            repo_root,
            "curio: finalize pending intake (resumed from prior partial run)",
        )?;
        eprintln!("Note: committed pending intake files left over from a prior partial run.");
    }
    Ok(())
}

/// Extract a Confluence folder ID from URLs of the form:
///   .../wiki/spaces/<KEY>/folder/<id>/...
fn extract_confluence_folder_id(url: &str) -> Option<String> {
    let idx = url.find("/folder/")?;
    let rest = &url[idx + "/folder/".len()..];
    let id: String = rest.chars().take_while(|c| c.is_ascii_digit()).collect();
...
mod url_tests {
    ...
    #[test]
    fn extracts_folder_id_from_example_url() {
        let url = "https://example.atlassian.net/wiki/spaces/fdeinterna/folder/4129456155";
        assert_eq!(
            extract_confluence_folder_id(url),
            Some("4129456155".to_string())
        );
        assert_eq!(extract_confluence_page_id(url), None);
    }

    #[test]
    fn extracts_page_id_from_pages_url() {
        let url = "https://x.atlassian.net/wiki/spaces/X/pages/12345/Title";
        assert_eq!(extract_confluence_page_id(url), Some("12345".to_string()));
        assert_eq!(extract_confluence_folder_id(url), None);
    }

    #[test]
    fn folder_url_with_trailing_segments_works() {
        let url = "https://x.atlassian.net/wiki/spaces/X/folder/9876543/some-name?param=1";
        assert_eq!(
            extract_confluence_folder_id(url),
