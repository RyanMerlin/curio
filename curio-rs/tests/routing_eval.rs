use curio::proposal::{ProposalLane, required_lane};
...
fn low_signal_meeting_notes_should_not_be_publishable() {
    ...
    assert!(
        !q.publishable,
        "meeting notes with TODO/TBD should not be publishable"
    );
    assert!(
        q.flags
            .iter()
            .any(|f| f.contains("placeholder") || f.contains("low_word")),
        "expected placeholder or low_word_count flag, got: {:?}",
        q.flags
    );
}
...
fn high_quality_upgrade_guide_is_publishable() {
    let title = "Example Server 2024.1 Upgrade Guide";
    let body = r#"
...
1. Download the 2024.1 installer from the vendor portal
...
"#;
    ...
    assert!(
        q.publishable,
        "detailed upgrade guide should be publishable; quality={:.2}, usability={:.2}, flags={:?}",
        q.information_quality, q.usability, q.flags
    );
    assert!(
        q.information_quality >= 0.45,
        "information_quality should be >= 0.45"
    );
...
fn hub_body_with_only_child_list_gets_low_quality_score() {
    ...
    let title = "Example Server";
    let body = "Hub page: Example Server\n\nThis page organizes the following sub-pages:\n\n- Upgrade Guides\n- Troubleshooting\n- Administration\n";
    let q = assess_quality(title, body);
    // Hub bodies are intentionally low-signal — they should not be staged directly.
    assert!(
        !q.publishable || q.information_quality < 0.6,
        "hub-only body should not pass the quality gate with high confidence; quality={:.2}",
        q.information_quality
    );
}
...
fn fixture_inputs_parse_correctly() {
    let fixture_dir =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/routing");
    ...
    for case in &cases {
        ...
        assert!(
            input_path.exists(),
            "fixture input.md missing for case '{}'",
            case
        );
        let page = curio::wiki_fs::parse_wiki_page(&input_path)
            .unwrap_or_else(|e| panic!("failed to parse fixture '{}': {}", case, e));
        assert!(
            !page.frontmatter.id.is_empty(),
            "id missing in fixture '{}'",
            case
        );
        assert!(
            !page.frontmatter.title.is_empty(),
            "title missing in fixture '{}'",
            case
        );
        assert_eq!(
            page.frontmatter.status.to_string(),
            "intake",
            "fixture '{}' should have intake status",
            case
        );
...
fn fixture_quality_gate_expectations() {
    ...
    let check = |case: &str, expect_publishable: bool| {
        ...
        if expect_publishable {
            assert!(
                q.publishable,
                "fixture '{}' should be publishable; q={:.2}, u={:.2}, flags={:?}",
                case, q.information_quality, q.usability, q.flags
            );
        } else {
            assert!(
                !q.publishable,
                "fixture '{}' should NOT be publishable; q={:.2}, u={:.2}",
                case, q.information_quality, q.usability
            );
        }
    };
