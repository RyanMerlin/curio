...
- `category` — e.g. `product-tree/example-server`
...
```json
{
  "routes": [
    {
      "slug": "my-page",
      "category": "product-tree/example-server",
      "status": "staged",
      "confidence": 0.87,
...
- Example Server ≠ Intelligence Suite — check product name carefully
...
```bash
curio query "What changed in Example Server 2024.1?" --json
```
